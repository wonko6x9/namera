...
- **Project scaffolding**
  ...
  - current honest state: Tauri bundle metadata and explicit Windows installer entry points now exist (`pnpm build:desktop`, `pnpm build:windows`), but a real Windows-host bundle run and installer smoke test are still needed before calling install readiness done
...
- **Safe local rename/move execution**
  ...
  - current honest state: first collision policy pass exists (`skip`, `overwrite`, `rename-new`), undo validates some stale-file cases, visible-batch apply now reports per-item outcomes honestly, subtitle sidecars preserve language qualifiers during rename planning, local batches now persist recovery state (planned/completed/failed inputs plus last processed item), integration coverage now exercises rename-new suffix selection, source-path reappearance during undo, and apply/undo/reapply cycles, the desktop flow now keeps a lightweight diagnostics log for provider/config/execution/recovery events, and the recovery panel now translates failed inputs plus recent execution diagnostics into concrete next-step guidance, but richer resume behavior and deeper diagnostics around recovery edges are still live seams
...
- **WebDAV destination backend**
  ...
  - current honest state: per-type WebDAV destination routing is now configurable and previewable, the desktop review now supports prompt-time local-vs-WebDAV destination preview selection, exported review plans now preserve the selected destination backend plus honest transfer contracts for both the full queue and the currently filtered visible subset, the desktop flow also exports a truthful WebDAV transfer queue for the visible set with a summary block, per-item ready/blocked state, concrete actions/reasons, ready-only plus blocked-only subsets, a deduplicated next-actions rollup, visible blocked-reason rollups, persisted queue snapshots for later review/handoff, a stable JSON export of the latest saved snapshot, and persisted transfer intents derived from saved snapshots with explicit next-action, blocker, prerequisite, derived handoff readiness, handoff owner/note, assignment timestamp, acknowledgement state, lifecycle event history, plus explicit recorded remote stage progress for manual mkdir/upload/verify work so downstream operators can track staged execution honestly, explicit blocked-stage reporting when mkdir/upload/verify gets stuck so handoff validation and owner packets stop pretending the lane is clear, and explicit reset-to-pending controls so a previously recorded stage can be retried without inventing fake success, plus an explicit control to mark blocked items resolved after snapshot review so handoff readiness can advance without recreating the intent, and a richer ready-operation handoff packet that separates ready remote actions from still-blocked items plus an explicit packet-readiness checklist (assigned, acknowledged, ready ops present, blocked items cleared, stage blockers surfaced) and a pass/fail validation export with concrete next-step remediation guidance, and now a condensed execution brief export that summarizes owner, ready targets, grouped mkdir/upload/verify operations, blockers, next steps, an owner-facing execution checklist, recorded stage progress, a machine-readable remote checklist packet, a concise handoff summary, a per-stage operation manifest with stage-specific per-item target detail plus by-kind counts, and an owner packet export with acknowledgement, required-action detail, ready-target counts by media kind, and stage-progress visibility for the downstream operator without pretending upload exists, plus latest and recent and acknowledged and handoff-ready packet exports, the status lane now summarizes which queued items are WebDAV-ready vs blocked overall, by media kind, and by grouped blocked reasons for triage, the review lane can now filter directly to WebDAV-ready vs blocked items, and phase 3 transfer contracts are previewed honestly (mkdir/upload/verify or blocked reasons), but transfer/upload execution remains unimplemented
