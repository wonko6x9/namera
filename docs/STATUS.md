...
- Phase 3 WebDAV routing is now configurable per library type (Movies / TV / Music) and previewable in the desktop flow, the review lane now lets the user switch preview mode between local destination paths and WebDAV destination/transfer contracts at prompt time instead of treating remote routing as all-or-nothing, the desktop review/export flow now emits a durable review plan set that records the selected destination backend plus the honest remote transfer contract preview (mkdir/upload/verify or blocked reasons), and the status lane now summarizes WebDAV transfer readiness counts plus blocked reasons across the current review set, but actual upload/copy execution is still not implemented
