...
- Phase 3 WebDAV routing is now configurable per library type (Movies / TV / Music) and previewable in the desktop flow, the review lane now lets the user switch preview mode between local destination paths and WebDAV destination/transfer contracts at prompt time instead of treating remote routing as all-or-nothing, the desktop review/export flow now emits both full and filter-respecting visible review plan sets that record the selected destination backend plus the honest remote transfer contract preview (mkdir/upload/verify or blocked reasons), it now also exports a truthful WebDAV transfer queue for the currently visible set with per-item ready/blocked state plus concrete actions/reasons as well as ready-only and blocked-only subsets, and the visible WebDAV queue now includes an explicit summary block in addition to per-item rows, while the status lane summarizes WebDAV transfer readiness overall, by media kind, and with grouped blocked reasons across the current review set. Actual upload/copy execution is still not implemented
