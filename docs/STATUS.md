...
- Provider request shaping exists, first live OMDb lookup wiring is present when an API key is configured, provider candidates now feed the preview-selection path, manual candidate override is wired into the desktop flow, local execution steps are modeled as explicit dry-run/apply/undo execution batches, and the Tauri side now exposes a native execution-batch preview command
