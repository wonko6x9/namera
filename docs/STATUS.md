...
## Current next step
Stay focused on Milestone 4 until local execution is boringly trustworthy: tighten batch sequencing, collision reporting, partial-success recovery, interruption-safe behavior, and Windows packaging readiness. The newest local slices are persisted batch recovery state, stronger exec-layer coverage for rename-new collisions, stale undo refusal when the source path reappears, apply/undo/reapply cycles, a lightweight diagnostics log for provider/config/execution/recovery troubleshooting, actionable recovery guidance that turns the latest batch state plus diagnostics into concrete next steps, a first honest Windows bundle/build surface (real Tauri bundle metadata, Windows icon wiring, and explicit `pnpm build:desktop` / `pnpm build:windows` entry points), and now a portable-first Tauri packaging path via `pnpm build:portable` plus corrected `beforeBuildCommand` wiring so real no-bundle builds actually run. `pnpm build:windows` now intentionally maps to the portable EXE/app-folder flow for initial installs, while installer packaging is pushed behind an explicit `pnpm build:windows:installer` entry point for later if it still earns its keep. The next likely move is a Windows-host smoke test of the portable build. WebDAV remote execution remains explicitly deferred as a low-priority later lane.
...
- Workspace scripts now expose `pnpm build:desktop`, `pnpm build:portable`, and `pnpm build:windows` so install-test work has obvious entry points, with `pnpm build:windows` now defaulting to the portable EXE/app-folder path and `pnpm build:windows:installer` reserved for explicit installer generation.
