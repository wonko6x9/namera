...
- Provider request shaping exists, first live OMDb lookup wiring is present when an API key is configured, provider candidates now feed the preview-selection path, manual candidate override is wired into the desktop flow, and local execution steps are rendered as an explicit action plan scaffold
