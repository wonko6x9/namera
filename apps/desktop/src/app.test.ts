...
describe("Namera MVP flow", () => {
  ...
  it("saves visible webdav queue snapshots through the controller", () => {
    const renders: string[] = [];
    const controller = createAppController((markup) => renders.push(markup));
    ...
    expect(loadWebdavTransferSnapshots()[0]?.filter).toBe("webdav-blocked");
  ...
  it("persists pending webdav transfer intents from the latest snapshot", () => {
    ...
    expect(renders.at(-1)).toContain('&quot;byKind&quot;');
    expect(renders.at(-1)).toContain('&quot;readyByKind&quot;');
    ...
    expect(renders.at(-1)).toContain('&quot;byKind&quot;');
    expect(renders.at(-1)).toContain('&quot;kind&quot;: &quot;movie&quot;');
    expect(renders.at(-1)).toContain('&quot;count&quot;: 3');
  ...
  it("exports plan sets and reports provider status honestly", () => {
    ...
    expect(exportWebdavTransferQueue([preview], {
      movieRoot: "Movies",
      tvRoot: "TV Shows",
      musicRoot: "Music",
      webdavMovieRoot: "/remote/movies",
    })).toContain('"state": "ready"');
    const queue = buildWebdavTransferQueue([preview], {
      movieRoot: "Movies",
      tvRoot: "TV Shows",
      musicRoot: "Music",
      webdavMovieRoot: "/remote/movies",
    });
    expect(queue[0]?.state).toBe("ready");
    expect(summarizeWebdavTransferQueue(queue).ready).toBe(1);
  ...
  it("summarizes webdav readiness honestly when some roots are configured", () => {
    ...
    expect(renders.at(-1)).toContain("movie: 1 ready, 0 blocked");
    expect(renders.at(-1)).toContain("episode: 2 ready, 0 blocked");
    expect(renders.at(-1)).toContain("unknown: 0 ready, 1 blocked");
